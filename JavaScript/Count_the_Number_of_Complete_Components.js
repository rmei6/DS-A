...
var countCompleteComponents = function(n, edges) {
  const extractConnectedSubgraph = function(edges) {
  ...
  const isComplete = function(graph, count = new Set(graph.flat()).size) { 
      return (count - 1) * count === 2 * graph.length;
  }

  const totallyDistinct = function(g1, g2, s2 = new Set(g2.flat())) {
      return !g1.flat().some(r => s2.has(r));
  }
